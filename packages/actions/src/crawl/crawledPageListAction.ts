...
export const crawledPageListInputSchema = z.object({
	sitemapId: z.string().uuid(),
	limit: z.number().int().positive().default(50),
	offset: z.number().int().nonnegative().default(0),
});
export const crawledPageListOutputSchema = z.object({
	pages: z.array(
		z.object({
...
export const crawledPageListContextSchema = baseActionContextSchema;

export const crawledPageListAction = async (params: {
	input: z.infer<typeof crawledPageListInputSchema>;
	ctx: z.infer<typeof crawledPageListContextSchema>;
}) => {
	...
	const sitemapUrlIds = sitemapUrlRows.map((r: any) => r.id);
	...
	const crawledPageIds = pageRows.map((p: any) => p.id);
	...
	const pages = pageRows.map((cp: any) => ({
		crawledPage: cp,
		analysis: analysisRows.find((a: any) => a.crawledPageId === cp.id) ?? null,
	}));
...
export const crawledPageListHandler = async (params: {
	input: z.infer<typeof crawledPageListInputSchema>;
	ctx: z.infer<typeof crawledPageListContextSchema>;
}) => {
	return crawledPageListAction(params);
};
