...
import { upsertCrawledPageBatchAction } from "../actions/crawl/upsertCrawledPageBatchAction";
import { upsertPageAnalysisBatchAction } from "../actions/crawl/upsertPageAnalysisBatchAction";
...
async function saveBatchResults(results: CrawlPageResult[]) {
	...
	const savedMap = new Map(saved.map((s) => [s.sitemapUrlId, s.id]));
